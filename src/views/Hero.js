...
export class Hero extends Component {
	render(){
		if (this.props.entry){
			return (
				<div className={`hero p-4 p-md-5 mb-4 text-white rounded ${this.props.isLight ? 'bg-dark' : 'bg-black'}`}>
					<h1 className="article-title display-4">
						<a href={'/article/'+this.props.entry.id}>{manip.sanitize(this.props.entry.title)}</a>
					</h1>
					<p className="lead fs-4">{manip.getText(this.props.entry.content, {count: 1, paras: false, images: false})}</p>
